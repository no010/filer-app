...
**收件箱**——下载完成自动入箱，规则匹配建议分类/目标/文件名；重复下载高亮提示，4 种处理（跳过 / 保留两者 / 替换 / 删除新文件）；键盘三连（j/k 移动、f 归档、i 跳过、e 编辑）；多选一键归档/删除。

![收件箱](screenshots/inbox.png)

**统计**——已归档/待办/近 7 天/待清理计数卡，按分类与陈旧度分布条形。

![统计](screenshots/stats.png)

> 截图为 HTML mock 渲染（`screenshots/mock/*.html` + `screenshots/screenshot.cjs`），非实拍；UI 以实际运行为准。
